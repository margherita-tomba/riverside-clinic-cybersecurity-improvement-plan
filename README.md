# Riverside Clinic Cybersecurity Improvement Plan

## Overview

This project applies the NIST Cybersecurity Framework (CSF) 2.0 to a fictional small outpatient healthcare clinic called Riverside Family Clinic.

The goal of the project was to:
- create a Current Profile and Target Profile,
- identify cybersecurity gaps,
- and develop a realistic improvement plan appropriate for a small healthcare provider.

The assessment focused on:
- ransomware resilience,
- patient data protection,
- vendor dependency,
- operational continuity,
- and incident response readiness.

## Simplified Clinic Network Architecture

![Simplified Clinic Network Architecture](images/clinic-network-architecture.png)

---

## Frameworks Used

- NIST Cybersecurity Framework (CSF) 2.0
- NIST SP 1300 Small Business Quick-Start Guide
- NIST SP 1301 Organizational Profiles

---

## Key Risks Identified

- Ransomware
- Phishing
- Weak MFA coverage
- Lack of tested restoration procedures
- Vendor dependency
- Incomplete incident response planning

![Risk Assessment Matrix](images/risk-assessment-matrix.png)

---

## Project Deliverables

- Clinic scope and assumptions
- Current Profile
- Target Profile
- Gap analysis
- Cybersecurity improvement plan
- Executive summary

---

## Skills Demonstrated

- Cybersecurity risk assessment
- Framework application
- Healthcare cybersecurity awareness
- Gap analysis
- Security governance
- Vendor risk analysis
- Incident response planning
- Security improvement prioritization

## Project Documents

- [Clinic scenario](01-clinic-scenario.md)
- [Assumptions and scope](02-assumptions-and-scope.md)
- [Current profile](03-current-profile.md)
- [Target profile](04-target-profile.md)
- [Gap Analysis](05-gap-analysis.md)
- [Executive summary](06-executive-summary.md)
- [References](references.md)
  
